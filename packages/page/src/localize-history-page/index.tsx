...
const LocalHistoryPage: FC = () => {
  ...
  const fetchHistory = () => {
    ...
    axios
      .get(
        `${process.env.NEXT_PUBLIC_BFF_API_URL}/history/conversations?userId=${localStorage.getItem(
          'userID',
        )}`,
        {
          headers: {
          ...
          return {
            ...
            secondaryAction: (
              <IconButton edge="end" aria-label="comments">
                {config?.allowDelete && (
                  <DeleteOutlineIcon onClick={() => deleteConversation(chatItem?.conversationId)} />
                )}
              </IconButton>
            ),
            onClick: handleClick,
