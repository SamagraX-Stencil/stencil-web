...
describe('FAQ component', () => {
  ...
  test('displays "Contact description" text', () => {
    const { getByText } = render(<FAQPage />);
    const textElement = getByText(config.contactDescriptionText ?? 'contact description');
    expect(textElement).toBeInTheDocument();
  });
  ...
  test('renders contact button correctly', () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(config.contactText ?? 'Contact User');
  });

  test('renders Manual User button correctly', () => {
    const consoleSpy = vi.spyOn(console, 'log');
    render(<FAQPage />);
    const buttonElement = screen.getByText(config.userManualText ?? 'User Manual');
    fireEvent.click(buttonElement);

    // Expect the console.log to be called with the expected value
    expect(consoleSpy).toHaveBeenCalledWith(config.userManualText ?? 'User Manual');
  });
});
